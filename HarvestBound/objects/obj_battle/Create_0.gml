...
function message_struct(_text, _sound = noone, _shake = false, _stop_audio = false) constructor
{
    text = _text;
    sound = _sound;
	screenShake = _shake
	stopAudio = _stop_audio
}
...
plyr = instance_find(obj_mainplayer,0)
...
{
	object_set_sprite(global.enemy[i], global.enemy[i].battle_sprite)
	global.enemy[i].x = 600
	global.enemy[i].y = 500
	
	global.enemy[i].rootX = 600 
	global.enemy[i].rootY = 500
	
	global.enemy[i].inBattle = true
}
